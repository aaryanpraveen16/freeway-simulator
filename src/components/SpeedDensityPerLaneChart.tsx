...
interface SpeedDensityPerLaneChartProps {
    dataHistory: any[];
    numLanes: number;
    unitSystem?: UnitSystem;
}

const SpeedDensityPerLaneChart: React.FC<SpeedDensityPerLaneChartProps> = ({
    dataHistory,
    numLanes,
    unitSystem = 'metric'
}) => {
    ...
    // Match colors from LaneThroughputChart
    const laneColors = [
    ...
    const chartData = useMemo(() => {
        return dataHistory.map(record => {
            ...
            if (record.laneSpeeds) {
                record.laneSpeeds.forEach((speed: number, i: number) => {
                    if (i < numLanes) {
                        point[`lane${i}`] = parseFloat(conversions.speed.toDisplay(speed).toFixed(2));
                    }
                });
            }
            return point;
    ...
    const stabilizedValues = useMemo(() => {
        ...
        for (let i = 0; i < numLanes; i++) {
            const laneSpeedData = dataHistory
                .filter(record => record.laneSpeeds && record.laneSpeeds[i] !== undefined)
                .map(record => conversions.speed.toDisplay(record.laneSpeeds[i]));

            results[`lane${i}`] = calculateStabilizedValue(laneSpeedData);
        }
    ...
    const handleExportImage = () => {
        ...
        try {
            ...
            downloadLink.download = "speed-per-lane-chart.svg";
        ...
        <Card>
            <CardHeader className="pb-2">
                <div className="flex justify-between items-center">
                    <div>
                        <CardTitle className="text-lg">Speed per Lane Over Time</CardTitle>
                        <p className="text-sm text-muted-foreground">
                            Analyze traffic flow characteristics for each lane individually
                        </p>
                    </div>
            ...
            <CardContent>
                <div className="h-[400px]" ref={chartRef}>
                    ...
                    >
                        ...
                        >
                            ...
                            <YAxis
                                name="Speed"
                                label={{ value: `Speed (${conversions.speed.unit})`, angle: -90, position: "insideLeft", style: { fontWeight: 500 } }}
                            ...
                            <Tooltip
                                formatter={(value: number, name: string) => {
                                    ...
                                    if (laneMatch) {
                                        return [
                                            `${value.toFixed(1)} ${conversions.speed.unit}`,
                ...
                <div className="mt-4 p-3 bg-gray-50 rounded-lg">
                    <h4 className="text-sm font-semibold mb-2">Stabilized Speed by Lane:</h4>
                            ...
                            <div key={i} className="flex justify-between items-center">
                                ...
                                <span className={`font-mono ${stabilizedValues[`lane${i}`]?.isStabilized ? 'text-green-600' : 'text-orange-600'}`}>
                                    {stabilizedValues[`lane${i}`]?.value ?
                                        `${stabilizedValues[`lane${i}`].value.toFixed(1)} ${conversions.speed.unit}` :
                                        'N/A'}
...
export default SpeedDensityPerLaneChart;
