...
const OverlaySpeedChart: React.FC<OverlaySpeedChartProps> = ({
  selectedSimulations
}) => {
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-center text-muted-foreground py-8">
            Select simulations to compare their speed patterns over time
          </div>
        </CardContent>
      </Card>
    );
        ...
        <div className="bg-white p-3 border border-gray-200 rounded-lg shadow-lg">
          ...
          {payload.map((entry: any, index: number) => (
            <p key={index} className="text-sm" style={{ color: entry.color }}>
              <span className="font-medium">{simulationNames[index]}:</span> {entry.value?.toFixed(1)} mph
            </p>
          ))}
        </div>
      );
    ...
    <Card>
      <CardHeader>
        ...
        <div className="text-sm text-muted-foreground">
          Overall average speed across {selectedSimulations.length} simulation{selectedSimulations.length !== 1 ? 's' : ''}
        </div>
      </CardHeader>
      <CardContent>
